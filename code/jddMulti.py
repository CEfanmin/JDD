...
def loadData(source_file, sum_file, target_file, pre_file):
    ...
    source_data_array = np.log1p(np.array(source_data)).reshape(90993,4,9,1)
    sum_data_array = np.log1p(np.array(sum_data)).reshape(90993,1,1,4)
    target__data_array = np.array(target_data).reshape(90993, 1, 1, 1)
    pred_data_array = np.log1p(np.array(pred_data)).reshape(90993, 4, 9, 1)
...
# ConvNet
month_input = Input(shape=(4,9, 1), dtype='float32',name='month_input')
month_input1 = layers.Conv2D(64,(3,3), activation='relu')(month_input)
month_input2 = layers.MaxPool2D(2,1)(month_input1)
...
month_input5 = layers.MaxPool2D(1,4)(month_input4)

# Residual Net
sum_input = Input(shape=(1,1,4), dtype='float32', name='sum_input')
sum_input1 = layers.Dense(16,activation='relu')(sum_input)
sum_input2 = layers.Dropout(0.2)(sum_input1)

sum_input3 = layers.Dense(16, activation='relu')(sum_input2)
sum_input4 = layers.Dropout(0.2)(sum_input3)

sum_input5 = layers.Dense(16, activation='relu')(sum_input4)
res = layers.add([sum_input5, sum_input2]) 

sum_input6 = layers.Dense(16,activation='relu',kernel_regularizer=regularizers.l2(l=0.001))(res)


# Concat
concatenated = layers.concatenate([month_input5, sum_input6], axis=-1)
...
# prediction
pre_sum_data = pd.read_csv('../data/userAbility_9-11.csv').replace([np.inf, -np.inf], 9999).fillna(0)
pre_sum_data_array = np.log1p(np.array(pre_sum_data)).reshape(90993,1,1,4)
